...
OUT="$REPO/ns3/results/blockchain_ablation"
RUNS="$OUT/runs"
SUM="$OUT/summary"
PLOTS="$OUT/plots"
...
make_trace () {
  ...
  local RUN_DIR="$REPO/sumo/output/bc_ablation/${TAG}"
...
# Ablation configs
TRUST_REPORTS="--enableReplayCheck=0 --enableSigCheck=0 --enableReports=1 --enableBlockchain=0 --enableTrustGate=1 --enableReplayAttack=0"
TRUST_BC="--enableReplayCheck=0 --enableSigCheck=0 --enableReports=1 --enableBlockchain=1 --enableTrustGate=1 --enableReplayAttack=0"

FULL_NO_BC="--enableReplayCheck=1 --enableSigCheck=1 --enableReports=1 --enableBlockchain=0 --enableTrustGate=1 --enableReplayAttack=1"
FULL_BC="--enableReplayCheck=1 --enableSigCheck=1 --enableReports=1 --enableBlockchain=1 --enableTrustGate=1 --enableReplayAttack=1"

echo "[STEP] Blockchain ablation runs..." >&2
for nveh in "${VEH_LIST[@]}"; do
  for spd in "${SPD_LIST[@]}"; do
    for seed in "${SEED_LIST[@]}"; do
      NS2="$(make_trace "$nveh" "$spd" "$seed")" || continue

      for model in trust_reports trust_bc full_no_bc full_bc; do
        TAG="bcabl_${model}_n${nveh}_s${spd}_seed${seed}"
        OUTCSV="$RUNS/${TAG}.csv"
        OUTEVT="$RUNS/${TAG}_events.csv"

        case "$model" in
          trust_reports) EXTRA="$TRUST_REPORTS" ;;
          trust_bc)      EXTRA="$TRUST_BC" ;;
          full_no_bc)    EXTRA="$FULL_NO_BC" ;;
          full_bc)       EXTRA="$FULL_BC" ;;
        esac
