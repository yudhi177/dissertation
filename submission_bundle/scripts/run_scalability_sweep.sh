...
OUT_BASE="$HOME/dissertation/ns3/results/scalability/runs"
...
for VEH in "${VEH_LIST[@]}"; do
  for SEED in "${SEEDS[@]}"; do

    TAG="veh_${VEH}_seed_${SEED}"
    CSV_OUT="${OUT_BASE}/${TAG}.csv"
    EVT_OUT="${OUT_BASE}/${TAG}_events.csv"

    echo "Running: vehicles=${VEH}, seed=${SEED}"
