...
# remove any older phase5 blocks (clean)
rm_blocks = [
...
// NOTE: g_confWindow and g_confMinForFast already exist -> DO NOT redeclare.
...
for p in targets:
    ...
    # Anchor: after cmd.AddValue("confMinForFast"...)
    m = re.search(r'cmd\.AddValue\(\s*"confMinForFast".*?\);\s*\n', txt)
    if not m:
        raise SystemExit(f"[ERR] confMinForFast AddValue anchor not found in {p}")
    ins = m.end()
    ...
    if 'cmd.AddValue("enableTrustConfidence"' not in txt:
        txt = txt.replace(m.group(0), m.group(0) + '  cmd.AddValue("enableTrustConfidence", "Enable confidence gating 0/1", g_enableTrustConfidence);\n', 1)

    if 'cmd.AddValue("enableChannelFairness"' not in txt:
        txt = txt.replace('cmd.AddValue("enableTrustConfidence", "Enable confidence gating 0/1", g_enableTrustConfidence);\n',
                          ...
                          '  cmd.AddValue("badChannelLossThresh", "Loss threshold for bad channel", g_badChannelLossThresh);\n'
