...
for f in glob.glob(os.path.join(RUNS, "DMAX_*_seed*.csv")):
    ...
    row = {"trustMaxAgeMs": dmax, "seed": seed,
           "staleMismatchRate": mismatchRate, "staleChecks": staleChecks}
    row.update(num)
    rows.append(row)
...
grp = raw.groupby("trustMaxAgeMs", as_index=False)

out = grp[metrics].agg(["mean","std","count"])
out.columns = ["_".join(c).strip("_") for c in out.columns]
out = out.reset_index().rename(columns={"index":"trustMaxAgeMs"})

# add ci95 columns
for m in metrics:
    out[m + "_ci95"] = grp[m].apply(ci95).values
