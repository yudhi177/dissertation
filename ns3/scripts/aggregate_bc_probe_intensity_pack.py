...
RUNS = os.path.expanduser("~/dissertation/ns3/results/bc_probe_intensity_pack/runs")
OUTD = os.path.expanduser("~/dissertation/ns3/results/bc_probe_intensity_pack/summary")
os.makedirs(OUTD, exist_ok=True)

pat = re.compile(r"^(BC_TRUST|BC_ALWAYS_QUERY)_probe(\d+)_seed(\d+)\.log$")
...
for logf in glob.glob(os.path.join(RUNS, "*.log")):
    ...
    queries=int(mm.group(1)); updates=int(mm.group(2))
    hits=int(mm.group(3)); misses=int(mm.group(4))
    hitRate=float(mm.group(5))
    avgQ=float(mm.group(6)); avgU=float(mm.group(7))
    totalOverheadMs = queries*avgQ + updates*avgU

    rows.append({
        "baseline": baseline, "bcProbeIntervalMs": probe, "seed": seed,
...
metrics=[c for c in raw.columns if c not in ("baseline","bcProbeIntervalMs","seed")]
grp = raw.groupby(["baseline","bcProbeIntervalMs"])
