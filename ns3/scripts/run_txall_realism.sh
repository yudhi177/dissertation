...
OUT="$REPO/ns3/results/txall_realism"
RUNS="$OUT/runs"
SUM="$OUT/summary"
PLOTS="$OUT/plots"
...
make_trace () {
  ...
  local RUN_DIR="$REPO/sumo/output/txall_realism/${TAG}"
...
echo "[STEP] TX_ALL realism runs..." >&2
for nveh in "${VEH_LIST[@]}"; do
  for spd in "${SPD_LIST[@]}"; do
    for seed in "${SEED_LIST[@]}"; do
      ...
      for txall in 0 1; do
        TAG="txall_n${nveh}_s${spd}_seed${seed}_tx${txall}"
        OUTCSV="$RUNS/${TAG}.csv"
        OUTEVT="$RUNS/${TAG}_events.csv"
        run_ns3 "$NS2" "$nveh" "$txall" "$OUTCSV" "$OUTEVT"
        echo "[OK] $TAG" >&2
      done
    done
  done
