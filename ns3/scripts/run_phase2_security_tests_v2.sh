...
cp -f /tmp/p2_rl.csv "$RUNS/" || true
cp -f /tmp/p2_rl_events.csv "$RUNS/" || true
...
cp -f /tmp/p2_rekey.csv "$RUNS/" || true
cp -f /tmp/p2_rekey_events.csv "$RUNS/" || true

echo "[3] Anti-downgrade test (forces downgrade attempt)"
./ns3 run "scratch/secure_trust_blockchain_v2x \
  --simTime=${SIM} \
  ...
  --enableAntiDowngrade=1 --enableDowngradeAttack=1 \
  --trustFastThresh=0.95 \
  --attackMode=2 --maliciousRate=0.2" | tee "$RUNS/p2_dg.log"
cp -f /tmp/p2_dg.csv "$RUNS/" || true
cp -f /tmp/p2_dg_events.csv "$RUNS/" || true
